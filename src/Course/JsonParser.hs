...
jsonString ::
  Parser Chars
jsonString = between (is doubleQuote) (charTok doubleQuote) (list chr)
  where
    doubleQuote = '"'
    ...
    handleEscape = do
      char <- character
      if char == 'u' then hex else handleSpecialCharacter char

    chr = do
      firstChar <- character
      if firstChar == '\\' then handleEscape
      else if firstChar == doubleQuote then unexpectedCharParser doubleQuote
      else pure firstChar
...
jsonNumber ::
  Parser Rational
jsonNumber = P parseFn
  where
    parseFn :: Chars -> ParseResult Rational
    parseFn inp = case readFloats inp of
      Empty -> UnexpectedString inp
      Full (a, rest) -> Result rest a
...
jsonObject ::
  Parser Assoc
jsonObject = betweenSepbyComma '{' '}' (lift3 (\a _ b -> (a, b)) jsonString (charTok ':') jsonValue)
...
jsonValue ::
  Parser JsonValue
jsonValue = spaces *> -- ignore initial spaces
  (JsonNull <$ jsonNull) |||
  (JsonTrue <$ jsonTrue) |||
  (JsonFalse <$ jsonFalse) |||
...
readJsonValue fp = (\content -> parse jsonValue content) <$> (readFile fp)
