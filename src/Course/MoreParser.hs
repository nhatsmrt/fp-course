...
tok parserA = do
  ...
  spaces
  pure a
-- tok parserA = lift2 (\a _ -> a) parserA spaces
...
option ::
  ...
  -> Parser a
  -> Parser a
option a parseA = parseA ||| (valueParser a)
...
between ::
  ...
  -> Parser a
  -> Parser a
between parserO parserC parserA = do
  parserO
  a <- parserA
  parserC
  pure a
...
hex ::
  Parser Char
hex = (\digits -> chr ((readHex digits) ?? 0)) <$> (replicateA 4 (satisfy isHexDigit))
...
hexu ::
  Parser Char
hexu = lift2 (\_ hexChars -> hexChars) (is 'u') hex
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 parserA parserS = lift2 (:.) parserA (list (lift2 (\_ a -> a) parserS parserA))
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby parserA parserS = (sepby1 parserA parserS) ||| (valueParser Nil)
...
eof = P matcher
  where
    matcher inp = case inp of
      Nil -> Result "" ()
      h :. _ -> UnexpectedChar h
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll preds = satisfy (\c -> and ((\pred -> pred c) <$> preds))
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny preds = satisfy (\c -> or ((\pred -> pred c) <$> preds))
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma start end parserA = betweenCharTok start end (sepby parserA (charTok ','))
