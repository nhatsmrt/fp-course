...
character ::
  Parser Char
character = P parserFn
  where
    parserFn input = case input of
      Nil -> UnexpectedEof
      h :. t -> Result t h
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  (<$>) f (P parserFn) = P (\inp -> f <$> (parserFn inp))
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(|||) (P parseFn1) (P parseFn2) = P (combine parseFn1 parseFn2)
  where
    combine parseFn1 parseFn2 inp =
      let result1 = parseFn1 inp in if isErrorResult result1 then parseFn2 inp else result1
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  (=<<) f (P parseFnA) = P (parseFn f)
    where
      parseFn f inp = case (parseFnA inp) of
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) parserF parserA = (\f -> (\a -> f a) <$> parserA) =<< parserF
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy pred = P (parseFn pred)
  where
    parseFn pred inp = case inp of
      Nil -> UnexpectedEof
      h :. t -> if pred h then Result t h else UnexpectedChar h
...
list ::
  Parser a
  -> Parser (List a)
list parserA = (list1 parserA) ||| (pure Nil)
...
list1 ::
  Parser a
  -> Parser (List a)
list1 parserA = lift2 (:.) parserA (list parserA)
-- list1 parserA = ((:.) <$> parserA) <*> (list parserA)
...
thisMany ::
  ...
  -> Parser a
  -> Parser (List a)
thisMany n parserA = sequenceParser (replicate n parserA)
...
-- firstNameParser =  (\a -> ((:.) a) <$> (list lower)) =<< upper
...
personParser ::
  Parser Person
personParser = lift5 (\a b c d e -> Person a b c d e) ageParserSpaced firstNameParserSpaced surnameParserSpaced smokerParserSpaced phoneParser
  where
    lift5 f a b c d e = (pure f) <*> a <*> b <*> c <*> d <*> e
    parseWithSpace a = lift2 (\a b -> a) a spaces1

    ageParserSpaced = parseWithSpace ageParser
    firstNameParserSpaced = parseWithSpace firstNameParser
    surnameParserSpaced = parseWithSpace surnameParser
    smokerParserSpaced = parseWithSpace smokerParser
